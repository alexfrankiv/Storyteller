...
console.log("Saved story");} else {
 ...
 exports.showSorted = function(req,res){
   console.log("Storing works"); StoryModel.find({}).sort({'title':1}).exec(function(err,stories){
        if(err){
            console.log("Not retrieved all stories");
        }else{
...
exports.showSortedAuthorAsc = function(req,res){
   console.log("Storing works"); StoryModel.find({}).sort({'author':1}).exec(function(err,stories){
        if(err){
            console.log("Not retrieved all stories");
        }else{
...
exports.showByAuthor = function (req, res){
    StoryModel.find({"author":req.params.author},function(err,stories){
        if(err){
...
exports.showByTitle= function(req, res){
 StoryModel.find({"title":req.params.title},function(err,stories){
        if(err){
...
exports.showByDescription = function(req,res){
  StoryModel.find({"description":req.params.description},function(err,stories){
        if(err){
...
exports.delete= function(req,res){
     StoryModel.remove({"_id":req.params.id}, function (err){
       if(err){
           console.log("Didn't delete anything");
       }else{
...
exports.update = function(req,res){
  StoryModel.findOneAndUpdate({ "_id":req.body._id},
                              {  title:req.body.title,
        root:req.body.root,
        author:req.body.author,
        description:req.body.description,
        genre:req.body.genre, },{upsert:true}, function(err,story){
        if(err){
console.log("have not found anything to update");}else{
